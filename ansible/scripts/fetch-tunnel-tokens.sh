...
STOCK_TOKEN="$(cd terraform && terraform output -raw cloudflare_tunnel_token_stock)"
FARMACIA_TOKEN="$(cd terraform && terraform output -raw cloudflare_tunnel_token_farmacia)"
...
sed -i \
  ...
  -e "s|^vault_cloudflare_tunnel_token_stock:.*|vault_cloudflare_tunnel_token_stock: \"${STOCK_TOKEN}\"|" \
  -e "s|^vault_cloudflare_tunnel_token_farmacia:.*|vault_cloudflare_tunnel_token_farmacia: \"${FARMACIA_TOKEN}\"|" \
  "$VAULT_FILE"
